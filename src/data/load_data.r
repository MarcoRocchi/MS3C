...
load_data <- function() {
    ...
    pre_operative["synchronous"] <- ifelse(pre_operative["Sinc 1/Meta 2"] == 0, 0.05255485, -0.39586723)
    pre_operative["metachronous"] <- ifelse(pre_operative["Sinc 1/Meta 2"] == 0, 0.17627974, 0.28472582)
    pre_operative["M"] <- ifelse(pre_operative["SEX"] == 1, -0.25222108, -0.21486229)
    pre_operative["F"] <- ifelse(pre_operative["SEX"] == 1, -0.96437500, -0.77958780)
    pre_operative["no"] <- ifelse(pre_operative["Malattia extrahep sinc fegato (0/1)"] == 0, 1.43862840, 1.86265170)
    pre_operative["sì"] <- ifelse(pre_operative["Malattia extrahep sinc fegato (0/1)"] == 0, 0.20406507, 0.22706775)
    ...
    surgery["morb severa sì"] <- ifelse(surgery["Morb severa"] == 1, -0.45834910, -0.36790693)
    surgery["morb severa no"] <- ifelse(surgery["Morb severa"] == 1, -0.77975430, -0.65975250)
    surgery["Infective morbidity sì"] <- ifelse(surgery["Infective morbidity (0/1)"] == 1, -0.43539820, 0.09909006)
    surgery["Infective morbidity no"] <- ifelse(surgery["Infective morbidity (0/1)"] == 1, 0.84917235, 0.65826590)
    surgery["r0 = Margine almeno 1 mm sì"] <- ifelse(surgery["r0 = Margine almeno 1 mm"] == 1, 0.61577220, 0.75887620)
    surgery["r0 = Margine almeno 1 mm no"] <- ifelse(surgery["r0 = Margine almeno 1 mm"] == 1, -0.18973102, -0.18494628)
